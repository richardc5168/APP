...
def q_u4_money_decimal_addsub(r: random.Random, qid: str, difficulty: str) -> Q:
    ...
    if scenario == "合計":
        ...
        price_strs = [_money2_from_cents(prices[i]) for i in range(k)]
        steps = [
            f"把金額小數點對齊：{' + '.join(price_strs)}。",
            f"相加得到合計：{' + '.join(price_strs)} = {_money2_from_cents(total)} 元。",
            f"答案：{_money2_from_cents(pay_total)} 元（小數點後兩位，可省略尾端 0）。",
        ...
        steps = [
            f"先算總價：{' + '.join(_money2_from_cents(prices[i]) for i in range(k))} = {_money2_from_cents(total)} 元。",
