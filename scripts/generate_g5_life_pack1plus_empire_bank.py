...
def q_u4_money_decimal_addsub(r: random.Random, qid: str, difficulty: str) -> Q:
    ...
    if scenario == "合計":
        ...
        price_strs = [_money2_from_cents(p) for p in prices]
        steps = [
        ...
        steps = [
            f"先算總價：{' + '.join(_money2_from_cents(p) for p in prices)} = {_money2_from_cents(total)} 元。",
            f"列式找零：{_money2_from_cents(pay)} − {_money2_from_cents(total)} = {_money2_from_cents(change)} 元。",
            f"答案：找零 {ans} 元。",
            f"檢查：{_money2_from_cents(pay)} > {_money2_from_cents(total)}，找零為正 ✓。",
        ]
        hints = [
            "先把所有價格加起來得到總價。",
            "找零 = 付的錢 − 總價。",
